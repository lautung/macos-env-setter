...
- **GUI 层**：生成单个 RunAtLoad LaunchAgent（`~/Library/LaunchAgents/<reverse-dns>.plist` + 工具生成的 `setenv.sh`），登录时按声明顺序重放 `launchctl setenv`。值中的 `$` 引用由该脚本按声明顺序展开；`$PATH` 锚点在此层解析为 agent 启动时继承到的 PATH（登录时即 launchd 默认 PATH）。
...
**期望值的环境**（实现期补充，票 #9）：回读自检与即时注入都在固定的 launchd-like 环境（HOME + launchd 默认 PATH）里跑脚本，与登录时 agent 重放的环境对齐——期望值只取决于配置，不取决于跑脚本的那个进程恰好继承到什么（`GuiLayer.launchdLikeEnvironment`）。代价是：会话中重跑 agent（重注册触发 `RunAtLoad`）时它继承到的是 gui 域的实时 PATH，`$PATH` 锚点会解析成那个值，与期望值不同，回读据此告警。
